...
rep('const VARIANT = "2d";', 'const VARIANT = "gl";')
...
rep("function resize() { DPR = Math.min(2, window.devicePixelRatio || 1); W = cv.clientWidth || 1280; H = cv.clientHeight || 720; U = Math.min(W, H) / 10; cv.width = W * DPR; cv.height = H * DPR; }   // 창이 숨겨져 크기가 0이어도 NaN 이 나지 않게 기본값",
...
rep('''  const zoom = 1 + .05 * bar + .28 * pulse(t, DROP, .01, .5) + .12 * pulse(t, TITLE, .01, .4) + .3 * pulse(t, POP, .01, .5);
  ...
  g.translate(W / 2 + (r0() - .5) * shake, H / 2 + (r0() - .5) * shake); g.scale(zoom, zoom);''',
    '''  g.translate(W / 2, H / 2);''')
i0 = s.index("  if (t >= POP - .38 && t < POP + .25) { const k = t < POP ? imp(t)")
i1 = s.index("/* ── 재생: 시계")
s = s[:i0] + "}\n\n" + "@@GL@@\n\n" + s[i1:]
...
function buildWaves() {
  waveList = [{ t0: DROP, x: 0, y: 0, r0: .5, sp: 12, dur: 1.5, str: .8 }, { t0: TITLE, x: 0, y: 0, r0: .5, sp: 13, dur: 1.4, str: .7 },
    { t0: POP - .38, x: 0, y: 0, r0: 8, sp: -20, dur: .38, str: .3 }, { t0: POP, x: 0, y: 0, r0: .2, sp: 18, dur: .85, str: 1.15 }];   // 펑: 빨려듦 → 폭발
...
function render(t) {
  ...
  burst(1, POP, 0, 0, 0, 7000, 2.0, 1.2, .1);                                           // 펑
  ...
  const lx = act2 ? 0 : yx, ly = act2 ? centerY(t) : yy, raysK = (act2 ? .16 + .3 * bp : .5 * seg(t, T0, T0 + 1) * (1 - seg(t, BUILD + 1, DROP))) * (1 - titleK * .6) + (t < POP ? .7 * imp(t) : .9 * pulse(t, POP, .01, .6));
  ...
  const hit = .02 * b1 + .04 * bar1 + .35 * pulse(t, DROP, .02, .6) + .05 * bp + .2 * pulse(t, TITLE, .02, .4) + .55 * pulse(t, POP, .02, .5) + .06 * seg(t, BUILD + 1, DROP) * (act2 ? 0 : 1);
  const zoom = 1 + .015 * b1 + .035 * bar1 + .04 * bar + .22 * pulse(t, DROP, .01, .5) + .1 * pulse(t, TITLE, .01, .4) + .26 * pulse(t, POP, .01, .45) - .05 * (t < POP ? imp(t) : 0);
  const flash = Math.max(pulse(t, DROP + .07, .01, .5), .8 * pulse(t, TITLE, .02, .4), 1.1 * pulse(t, POP + .06, .01, .42), .5 * pulse(t, T0, .01, .3)), fade = Math.max(1 - seg(t, 0, .25), seg(t, FADE, END));
  ...
  X.pass(P.final, null, (u) => { X.bindTex(0, RT.scene.tex, u.uScene); X.bindTex(1, bloomTex, u.uBloom); X.bindTex(2, raysTex, u.uRaysTex); gl.uniform2f(u.uRes, pw, ph); gl.uniform1f(u.uU, Ud); gl.uniform1f(u.uT, t);
    gl.uniform2f(u.uShake, (r0() - .5) * hit * U / W, (r0() - .5) * hit * U / H); gl.uniform1f(u.uZoom, zoom); gl.uniform1f(u.uCA, .001 + .011 * hit + .004 * bp * (act2 ? 1 : 0)); gl.uniform1f(u.uBloomK, .55 + .25 * bp + .2 * b1 + .6 * (t < POP ? imp(t) : 0));
    gl.uniform1f(u.uFlash, flash * flash * .85); gl.uniform1f(u.uInvert, (t >= DROP && t < DROP + .07) || (t >= POP && t < POP + .06) ? 1 : 0); gl.uniform1f(u.uFade, fade); gl.uniform4fv(u.uWaves, wv); });
}
'''
s = s.replace("@@GL@@", GL)

# ── 재생부: draw → render, 느린 GPU 대응 (WebGL 초기화 실패는 prepare 에서 throw → 키트가 안내)
rep("function init() {\n  buildRot();", "function init() {\n  buildRot(); buildWaves();")
rep("  frame(t, dt, raw) { time = t; update(t, dt); draw(t); },",
...
rep("{ resize(); simulate(time); draw(time); } });", "{ resize(); simulate(time); render(time); } });")
rep("function renderStill(tt) { simulate(tt); draw(tt); }", "function renderStill(tt) { simulate(tt); render(tt); }")
