/* ═══════════════════════════════════════════════════════════════
   ...
   player.play({ sound, from, onDone })  ·  player.still(t)  ·  player.active()
   주소: ?autoplay=1&sound=0|1 (갤러리 iframe — 끝나면 parent 로 { type: "cutscene-done" }), ?t=12.4 (정지 화면)
...
function create(cfg) {
  ...
  let opts = {}, running = false, paused = false, on = false, time = 0, anchor = 0, raf = 0, useAudio = false, soundOn = true, idleTimer = 0, curCap = null, lastSec = -1, lastChap = -2, dragging = false;
  ...
  wrap.innerHTML = `
    ...
    <div class="csui-end" hidden>
      <div class="csui-kicker">Fin</div><h2>${esc(cfg.title)}</h2><p>${esc(cfg.sub || "")}</p>
      <div class="csui-actions"><button class="csui-play" data-act="replay">${ICON.replay}다시 보기</button><button class="csui-ghost" data-act="home">처음 화면</button><a class="csui-ghost" href="${gallery}">컷씬 목록</a></div>
    </div>`;
  ...
  const capEl = el(".csui-cap"), capP = el(".csui-cap p"), capRule = el(".csui-cap i"), fill = el(".csui-fill"), knob = el(".csui-knob"), tip = el(".csui-tip"), mini = el(".csui-mini i"),
    timeB = el(".csui-time b"), chapEl = el(".csui-chap"), track = el(".csui-track"), toast = el(".csui-toast"), endEl = el(".csui-end"), btnMute = el('[data-act="mute"]'), btnPause = el('[data-act="pause"]');
  ...
  function play(o = {}) {
    ...
    Promise.race([audioReady, new Promise((r) => setTimeout(r, 2500))]).then(() => {
      ...
      anchor = now() - from; running = true; startAudio(from); cancelAnimationFrame(raf); raf = requestAnimationFrame(tick);
    });
  }
  function halt() { playToken++; running = false; cancelAnimationFrame(raf); if (audio) { try { audio.pause(); } catch (e) { /* 무시 */ } } }
  function finish() {
    halt(); paused = false; capEl.style.opacity = 0;
    if (embedded || opts.onDone) { stage.hidden = true; on = false; if (opts.onDone) opts.onDone(); if (embedded) post(); return; }
  ...
  function close() { if (embedded || opts.onDone) { halt(); stage.hidden = true; on = false; if (opts.onDone) opts.onDone(); if (embedded) post(); } else home(); }
  ...
  stage.addEventListener("click", (e) => {
    ...
    ({ back: close, skip: () => (embedded || opts.onDone ? close() : finish()), mute: toggleSound, full: toggleFull, pause: togglePause, replay: () => play({ sound: soundOn }), home })[act.dataset.act]();
  });
  ...
  const api = { play, still, seek, close, active: () => on, time: () => time, land, stage };

  stage.hidden = true;
  setTimeout(() => {                                   // 페이지 스크립트가 끝난 뒤에 주소 옵션 처리
    const qt = q.get("t");
    if (embedded) { land.hidden = true; play({ sound: q.get("sound") !== "0" }); }
